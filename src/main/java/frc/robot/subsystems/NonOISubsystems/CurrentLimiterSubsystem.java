...
public class CurrentLimiterSubsystem extends SubsystemBase {
    ...
    public void LimitCurrent(double currentCurrent) {
        if (Arrays.stream(limitedMotors).sum() != 13 && currentCurrent > currentLimit) {
            limitedMotors[12 - Arrays.stream(limitedMotors).sum()] = 1; //sets motors to limited in increasing order of importance
        }
        if (Arrays.stream(limitedMotors).sum() != 0 && currentCurrent < currentLimit){
            limitedMotors[13 - Arrays.stream(limitedMotors).sum()] = 0; //sets motors to not limited in decreasing order of importance
        }
        for (int i : motorID) {
            switch(i){
                case 1:
