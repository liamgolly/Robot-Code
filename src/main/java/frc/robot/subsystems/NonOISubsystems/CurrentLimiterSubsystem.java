...
public class CurrentLimiterSubsystem extends SubsystemBase {
    ...
    public boolean limitMotor(String group, boolean limit) {
        switch (group) {
            case "driveTrain":
                if (limit) {
                    if (limitDrivetrain == 1) {
                        return false;
                    } else {
                        limiter(leftMotor1ID, limitDrivetrain + xStep);
                        limiter(leftMotor2ID, limitDrivetrain + xStep);
                        limiter(rightMotor1ID, limitDrivetrain + xStep);
                        limiter(rightMotor2ID, limitDrivetrain + xStep);
                        limitDrivetrain = limitDrivetrain + xStep;
                        return true;
                    }
                } else {
                    limiter(leftMotor1ID, limitDrivetrain - xStep);
            ...
            case "climb":
                if (limit) {
                    if (limitClimb == 1) {
                        ...
                        limiter(pvcExtenderID, limitClimb + xStep);
                        limiter(rightWinchID, limitClimb + xStep);
                        limiter(leftWinchID, limitClimb + xStep);
                        limitClimb = limitClimb + xStep;
                        return true;
                    }
                } else {
                    limiter(pvcExtenderID, limitClimb - xStep);
            ...
            case "turret":
                if (limit) {
                    if (limitTurret == 1) {
                        ...
                        limiter(turretRotatorID, limitTurret + xStep);
                        limiter(shooterID, limitTurret + xStep);
                        limitTurret = limitTurret + xStep;
                        return true;
                    }
                } else {
                    limiter(turretRotatorID, limitTurret - xStep);
            ...
            case "ballTransport":
                if (limit) {
                    if (limitBallTransport == 1) {
                        return false;
                    } else {
                        limiter(intakeID, limitBallTransport + xStep);
                        limiter(intakeRotatorID, limitBallTransport + xStep);
                        limiter(turretIntakeID, limitBallTransport + xStep);
                        limiter(hopperID, limitBallTransport + xStep);
                        limitBallTransport = limitBallTransport + xStep;
                        return true;
                    }
                } else {
                    limiter(intakeID, limitBallTransport - xStep);
            ...
            case "colorWheel":
                if (limit) {
                    if (limitColorWheel == 1) {
                        return false;
                    } else {
                        limiter(colorWheelID, limitColorWheel + xStep);
                        limitColorWheel = limitColorWheel + xStep;
                        return true;
                    }
                } else {
                    limiter(colorWheelID, limitColorWheel - xStep);
